...
const QRGenerator = ({ job, type, qrCodeData, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-white border border-gray-200 rounded-2xl max-w-sm w-full p-6 text-center space-y-6 shadow-2xl">
        <div>
          <h3 className="text-lg font-bold text-[#06402B]">QR {type === 'check-in' ? 'Check-In' : 'Check-Out'}</h3>
        ...
        <button 
          onClick={onClose}
          className="w-full bg-gray-100 hover:bg-gray-700 text-[#06402B] font-semibold py-2 px-4 rounded-lg border border-gray-200 transition text-xs"
        >
          Close Code
        </button>
      </div>
