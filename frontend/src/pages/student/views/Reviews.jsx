        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {reviews.map((review) => {
            ...
            return (
              <div 
                key={review.id} 
                className="bg-white border border-gray-200 rounded-xl p-5 space-y-4 hover:border-emerald-700/40 hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-[10px] text-gray-500 mt-1">
                      ...
                      <span className="text-[#06402B] font-semibold">{partner?.name || 'Unknown User'}</span>{' '}
                      ({partner?.email || 'N/A'})
                    </div>
                  </div>
                ...
                {review.job && (
                  <div className="flex justify-between items-center text-[10px] text-gray-500 pt-2 border-t border-gray-200">
                    ...
                    <span className="text-emerald-700 font-bold">LKR {review.job.payAmount || 'N/A'}</span>
                  </div>
                )}
              </div>
            );
